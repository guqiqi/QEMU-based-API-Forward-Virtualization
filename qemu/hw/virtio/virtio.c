...
{
    uint64_t addr;
    uint32_t len;   // size of buffer, number of descriptor = len / sizeof(VRingDesc)
    uint16_t flags;
    uint16_t next;
} VRingDesc;
...
typedef struct VRingAvail
{
    uint16_t flags;
    uint16_t idx;
    uint16_t ring[];    //descriptor of buffer head in vring array
} VRingAvail;
...
typedef struct VRingUsedElem
{
    uint32_t id;
    uint32_t len;
} VRingUsedElem;  // one VirtQueueElement correspond to one VRingUsedElem
...
{
    unsigned int num; // number of descriptor in descriptor table
    unsigned int num_default;
    unsigned int align;
    hwaddr desc;    // pointer to descriptor table
    hwaddr avail;   // pointer to VRingAvail
    hwaddr used;    // pointer to VRingUsed
...
{
    VRing vring;
    VirtQueueElement *used_elems; // guest write sth to host, append one or more virtqueueelement object

    /* Next head to pop */
    uint16_t last_avail_idx;    // last used buffer head in ring[](VRingAvail), next avail = last_avail_idx++
